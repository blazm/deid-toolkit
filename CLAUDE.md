...
root_dir/                      # Data directory (user extension points)
  ...
  datasets/                    # original/, aligned/, labels/, pairs/, deidentified/
    original/{dataset-name}/           # Raw images (needs MTCNN alignment first)
    aligned/{dataset-name}/            # Pre-aligned face crops (ready for pipeline)
    labels/{dataset-name}_labels.csv   # Identity labels per dataset
    pairs/*_genuine_pairs.txt          # Generated by preprocess step
    deidentified/{technique}/{ds}/     # Technique output images

**Dataset naming convention**: Use the canonical dataset name as the folder name. Examples:
- `original/arface/`, `original/lfw/`, `original/utkface/` — raw originals
- `aligned/arface/`, `aligned/lfw/`, `aligned/celeba-test/` — post-MTCNN
The wizard (`deid select wizard`) auto-discovers unaligned datasets from original/ and offers to align them.
- `aligned/arface_aligned/`, `aligned/lfw_aligned/`, `aligned/celeba-test/` — post-MTCNN
The wizard (`deid select wizard`) auto-discovers unaligned datasets from original/ and offers to align them.
