...
PATH_TO_MODEL_WEIGHTS  = str(SCRIPT_DIR / "weights" / "VGG_FACE.t7")
...
def main():
    ...
    detection_utility = []
    for i, (name_a, name_b, gt_label) in enumerate(tqdm(zip(names_a, names_b, ground_truth_binary_labels), total=len(names_a), desc=f"vggface | {dataset_name}-{technique_name}")):
