"""Generate genuine/impostor image pairs from label CSV files.

Ported from modules/utils/generate_img_pairs_all.py.
"""
import csv
import os
import itertools
    ...
    for dataset_name in selected_datasets_names:
        ...
        with open(genuine_pairs_savepath, "w+") as genuine_pairs_file:
            for pair in genuine_pairs:
                id1 = [
                    id for id, images in identity_clusters.items()
                    if pair[0] in images
                ][0]
                id2 = [
                    id for id, images in identity_clusters.items()
        ...
        if len(identity_clusters) < 2:
            print(
                f"Not enough identities to generate impostor pairs for {dataset_name} dataset."
            )
        else:
            impostor_pairs = []
