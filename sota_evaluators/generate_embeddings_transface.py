#!/usr/bin/env python3
"""
Generate face embeddings using TransFace.

Usage:
    python generate_embeddings_transface.py \
        --input <folder_with_face_images> \
        --output <folder_for_embeddings> \
        --weight <path_to_model>.pt \
        --network vit_s_dp005_mask_0

Pretrained Models (pick one):
    TransFace-S (MS1MV2):  https://drive.google.com/file/d/1UZWCg7jNESDv8EWs7mxQSswCMGbAZNF4/view
    TransFace-L (MS1MV2):  https://drive.google.com/file/d/1uXUFT6ujEPqvCTHzONsp6-DMIc24Cc85/view
    TransFace-S (Glint360K): https://drive.google.com/file/d/18Zh_zMlYttKVIGArmDYNEchIvUSH5FQ1/view
    TransFace-L (Glint360K): https://drive.google.com/file/d/1jXL_tidh9KqAS6MgeinIk2UNWmEaxfb0/view

    Place downloaded model in: models/transface/glint360k_model_TransFace_L.pt
    (or specify any path with --weight)

Conda Environment:
    conda env create -f environment_transface.yml
    conda activate transface

Network Selection (--network):
    For MS1MV2  TransFace-S / Glint360K TransFace-S:  vit_s_dp005_mask_0
    For MS1MV2  TransFace-L / Glint360K TransFace-L:  vit_l_dp005_mask_005

Example:
    python generate_embeddings_transface.py \
        --input ./face_images/ \
        --output ./transface_embeddings/ \
        --weight models/transface/glint360k_model_TransFace_L.pt \
        --network vit_s_dp005_mask_0
"""
...
# Add TransFace directory to path for imports
TRANSFACE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "TransFace")
sys.path.insert(0, TRANSFACE_DIR)

from backbones import get_model


def preprocess_image(img_path, target_size=(112, 112)):
    """Load and preprocess a single image for TransFace.
...
def extract_embeddings(image_paths, model, device, batch_size=32):
    ...
    for start in range(0, total, batch_size):
        ...
        # TransFace model forward returns: (embedding, weight, patch_entropy)
        emb, _, _ = model(batch)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate face embeddings using TransFace"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=os.path.join(os.path.dirname(os.path.abspath(__file__)),
                             "models", "transface", "model.pt"),
        help="Path to TransFace model weights (.pt file)"
    )
    parser.add_argument(
        "--network", type=str, default="vit_s_dp005_mask_0",
        choices=["vit_s_dp005_mask_0", "vit_l_dp005_mask_005"],
        help="Network architecture (default: vit_s_dp005_mask_0 for TransFace-S)"
    )
    parser.add_argument(
    ...
    # Resolve paths — preserve last 2 folder levels from input in output
    ...
    print(f"TransFace Embedding Generator")
    print(f"=============================")
    ...
    if not os.path.exists(weight_path):
        ...
        print()
        print("Download a pretrained model from:")
        print("  TransFace-S (MS1MV2):")
        print("    https://drive.google.com/file/d/1UZWCg7jNESDv8EWs7mxQSswCMGbAZNF4/view")
        print("  TransFace-L (Glint360K):")
        print("    https://drive.google.com/file/d/1jXL_tidh9KqAS6MgeinIk2UNWmEaxfb0/view")
        print()
    ...
    model = get_model(args.network, fp16=False, num_features=512)
