#!/usr/bin/env python3
"""
Batch-run TransFace embedding generation across multiple dataset folders.
...
Usage:
    python batch_run_transface.py ^
        ...
        --output-root  D:\dev\deid-toolkit\root_dir\embeddings\TransFace
"""
...
def main():
    parser = argparse.ArgumentParser(description="Batch TransFace embedding generation")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="TransFace model weights path"
    )
    parser.add_argument(
        "--network", type=str, default="vit_l_dp005_mask_005",
        choices=["vit_s_dp005_mask_0", "vit_l_dp005_mask_005"],
        help="Network architecture (default: vit_l_dp005_mask_005)"
    )
    parser.add_argument(
    ...
    embed_script = os.path.join(script_dir, "generate_embeddings_transface.py")

    if args.weight is None:
        args.weight = os.path.join(script_dir, "models", "transface", "glint360k_model_TransFace_L.pt")
    ...
    print(f"TransFace Batch Embedding Generation")
    print(f"=====================================")
