...
import cmd, sys # cmd is a module to create line-oriented command interpreters
...
class DeidShell(cmd.Cmd):
    ...
    def __init__(self): #, completekey: str = "tab", stdin: os.IO[str] | None = None, stdout: os.IO[str] | None = None) -> None:
    ...
    def do_datasets(self, arg):
        ...
        if self.root_dir is None:
            ...
            if os.path.exists(datasets_path):
                ...
                print(Fore.RED + 'Datasets directory not found. Does the ROOT_DIR ({0}) have folder named "datasets"?'.format(self.root_dir), Fore.RESET)
    ...
    def do_select(self, arg):
        ...
        switcher['*'] = lambda arg : [switcher[option](arg) for option in switcher.keys()] # run all
    ...
    def select_datasets(self, arg): 
        ...
        if self.root_dir is None:
            ...
            if os.path.exists(datasets_path):
                ...
                selected_datasets = input("Selection: ").split() # TODO: remember selected datasets
                for i in selected_datasets:
                    try:
                        print(Fore.LIGHTYELLOW_EX + '\t' + str(i) + '. ' + datasets[int(i)], Fore.RESET)
                    except:
        ...
        if self.root_dir is None:
            ...
            if os.path.exists(techniques_path):
                ...
                for i in selected_techniques:
                    try:
                        print(Fore.LIGHTYELLOW_EX + '\t' + str(i) + '. ' + techniques[int(i)], Fore.RESET)
                    except:
                ...
                print(Fore.RED + 'Techniques directory not found. Does the ROOT_DIR ({0}) have folder named "techniques"?'.format(self.root_dir), Fore.RESET)
        ...
        if self.root_dir is None:
            ...
            if os.path.exists(evaluation_path):
                ...
                for i in selected_evaluation:
                    try:
                        print(Fore.LIGHTYELLOW_EX + '\t' + str(i) + '. ' + evaluation[int(i)], Fore.RESET)
                    except:
                ...
                self.config.set('selection', 'evaluation', ' '.join(selected_evaluation))
                # save config
                with open('config.ini', 'w') as configfile:
                    self.config.write(configfile)
            else:
                print(Fore.RED + 'Evaluation directory not found. Does the ROOT_DIR ({0}) have folder named "evaluation"?'.format(self.root_dir), Fore.RESET)
    ...
    def default(self, line):
        ...
        switcher = {
            'config': lambda : print({section: dict(self.config[section]) for section in self.config.sections()}),
