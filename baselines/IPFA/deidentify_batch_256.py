#!/usr/bin/env python
"""Batch face de-identification using StarGAN 256x256 CelebA Generator.

FAO / IPFA (Li et al., ACM MM 2021): Identity-preserving face anonymization
via facial attributes obfuscation. Uses the original StarGAN generator trained
on CelebA 256x256 with 5 label attributes (Black Hair, Blond Hair, Brown Hair,
Male, Young), changing visual appearance while preserving identity discriminability.

Pipeline per image:
  Image -> resize to 256x256 -> normalize [-1,1] -> StarGAN Generator
  (with reversed attribute vector) -> de-identified output at 256x256

Model requirements:
  - pretrained/celeba-256x256-5attrs/200000-G.ckpt (original CelebA 256x256 weights)

Usage:
    python deidentify_batch_256.py --input <source_folder> --output <dest_folder>
"""
...
# StarGAN attributes (c_dim=5) — original CelebA 256x256 label attributes
...
class Generator(torch.nn.Module):
    """StarGAN Generator (CelebA 256x256, c_dim=5, repeat_num=6)."""
...
def download_model_if_needed(model_path: Path):
    """Check for StarGAN 256x256 CelebA generator checkpoint."""
    if model_path.exists():
        print(f"  Using existing generator: {model_path}")
        return
    model_path.parent.mkdir(parents=True, exist_ok=True)
    print(f"  Generator not found at {model_path}. Please place celeba-256x256-5attrs/200000-G.ckpt there.")
    sys.exit(1)
...
def load_generator(model_path: Path, device):
    ...
    # Keep in training mode — per-instance normalization, same as 128 version
    return G


def preprocess_image(image: Image.Image):
    """Transform PIL image to tensor [-1, 1] matching StarGAN training transform (256x256)."""
    transform = T.Compose([
        T.Resize(256),
        T.ToTensor(),
...
def run(args):
    ...
    # Load StarGAN Generator (CelebA 256x256 weights)
    model_path = REPO_ROOT / "pretrained" / "celeba-256x256-5attrs" / "200000-G.ckpt"
    download_model_if_needed(model_path)

    print("Loading StarGAN generator (256x256)...")
    ...
    # AttributeNet uses different attribute space than original CelebA 256x256.
    ...
    for image_path in tqdm(image_files, desc="De-identifying"):
        try:
            ...
            # Convert to image (already 256x256 — no post-upsampling needed)
            out_np = (best_out.cpu().numpy().transpose(1, 2, 0) * 255).astype(np.uint8)
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch face de-identification using StarGAN 256x256 CelebA Generator",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("--input", required=True, help="Input folder with aligned face images")
    parser.add_argument("--output", required=True, help="Output folder for de-identified images (PNG, 256x256)")
